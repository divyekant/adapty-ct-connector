...
- **Webhook auth is per tenant.** The API Gateway TOKEN authorizer (`cmd/authorizer`) derives the tenant from the method ARN and validates against SSM parameter `/adapty-ct-connector/{env}/{tenant}/auth-token`. Never reintroduce a global token: one tenant's onboarding/rotation must not affect another, and a token must only work on its own `/ingest/{tenant}` path.
...
- **Records carry exactly one identity key.** When the Adapty custom attribute named by `clevertap_id_attribute` (default `clevertap_id`) is present, records are keyed by CleverTap `objectId`; otherwise by `identity` (`customer_user_id` → `profile_id`). Never send both keys on one record — CleverTap resolves them independently and a mix splits profiles.
